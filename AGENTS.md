...
- **Master**（`src/master/`）：唯一中枢服务，HTTP `/api` + WS `/ws` + 内嵌 Dashboard 都挂在 `:28800`，用 SQLite WAL 持久化群 / Issue / 协作 / 消息 / 记忆 / 会话。
...
- **数据主路径**：Worker WS → `src/master/ws-hub/connection.ts` 派发 → `src/master/ws-hub/{routing,collaboration,sessions,directory}.ts` → SQLite（`src/master/db/{core,internal,agents,groups,issues,messages,collaboration,ask-bridges,...}.ts`）→ 推送给目标 client；离线收件人走 `src/master/offline-queue.ts`（100 条 / 24h TTL）。`ws-hub.ts` 是 `db/index.ts` 风格的 facade，下层 `internal.ts` 装载 6 个方法模块。
...
- **Collaboration**：`collab create` 写 `collaborations` 表 → 派 `collaboration_started` WS → first speaker kickoff → 跟踪 `trackCollaborationTurn` / 收尾 `concludeCollaboration` → 派 `collaboration_concluded`。`enrichConversationWithCollaboration` 把 workingDir / activeIssues / collab 元数据挂到 outgoing conversation。
...
| Collaboration | `src/master/ws-hub/collaboration.ts` + `src/master/api/groups.ts` 的 `/collaborations` | `collab create` / `conclude` |
...
| Real persons | `src/shared/protocol.ts` 的 `REAL_PERSONS` + `/api/real-persons` | dashboard 协作 owner 选择 |
...
| `src/master/db/{core,types,agents,groups,issues,messages,collaboration,ask-bridges,schedules,schedule-patterns,memory,skills,notes,agent-sessions,issues-patrol,domains,guidance-templates}.ts` | SQLite 领域表操作（每个文件 = 一类表） |
| `src/master/ws-hub/index.ts` → `internal.ts` | WS Hub facade + composition root |
| `src/master/ws-hub/{connection,routing,directory,sessions,collaboration}.ts` + `dispatch-enrich.ts` | WS Hub 6 个方法模块 |
...
| `src/executor/worker-chat.ts` | 群聊 reply + 协作 kickoff + 同群串行队列 |
...
| `src/cli/{common,config,identity,directory,group,issue,collab,ask,note,memory,skill,schedule,master,executor,init}.ts` | 各子命令实现 |
...
- REST API 中间件 permissive —— Dashboard cookie session 与 CLI `Authorization: Bearer mesh_xxx` 都放行。CLI token 解析注入到 `req.agentAuth`，管理端点（share mint、agent update、collab create 等）会 `requireAgent` 校验。
