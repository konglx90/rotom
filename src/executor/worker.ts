...
import { resolveIssueCwd as resolveIssueCwdImpl, cleanupIssueWorktrees as cleanupIssueWorktreesImpl } from "./worker-worktrees.js";
...
export class ExecutorWorker {
  ...
  async resolveIssueCwd(
    ...
    // 实现已抽到 ./worker-worktrees.ts(只依赖 workingDir / workingDirMap / tag)。
    return resolveIssueCwdImpl(
      { workingDir: this.workingDir, workingDirMap: this.config.workingDirMap, tag: this.tag },
      groupId, override, repoCtx,
    );
  }
  ...
  cleanupIssueWorktrees(
    ...
    cleanupIssueWorktreesImpl(
      { workingDir: this.workingDir, workingDirMap: this.config.workingDirMap, tag: this.tag },
      groupId, issueId, repoCtx,
    );
  }
