 ...
 *     to auth / heartbeat / a2a_* / update_info / disconnect / issue_update /
 *     issue_approval_request / session_* handlers. a2a chat/reply 四个分支
 *     (a2a_send / a2a_reply / a2a_reply_chunk / a2a_reply_end)已拆到
 *     ./connection-chat.ts,这里一行 dispatch 过去。
...
// ReplyContext + resolveReplyContext 已移到 ./connection-chat.ts(a2a reply 处理器专用)。
  ...
  handleConnection(this: WSHubSelf, ws: WebSocket, req: IncomingMessage): void {
    ...
    ws.on("message", (raw) => {
      ...
      if (msg.type === "a2a_send") {
        this.handleA2aSend(ws, agentId, msg);
        return;
      }

      if (msg.type === "a2a_reply") {
        this.handleA2aReply(agentId, msg);
        return;
      }

      if (msg.type === "a2a_reply_chunk") {
        this.handleA2aReplyChunk(agentId, msg);
        return;
      }

      if (msg.type === "a2a_reply_end") {
        this.handleA2aReplyEnd(agentId, msg);
        return;
      }
