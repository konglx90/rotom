...
export class WSHub {
  ...
  private handleConnection(ws: WebSocket): void {
    ...
    ws.on("message", (raw) => {
      ...
      if (msg.type === "group_history_request") {
        const conn = this.connections.get(agentId);
        const agentName = conn?.name || "unknown";

        // Verify agent is a member of the group
        const members = this.db.getGroupMembers(msg.groupId);
        const isMember = members.some(m => m.agent_name === agentName);
        if (!isMember) {
          this.send(ws, {
            type: "group_history_response",
            requestId: msg.requestId,
            messages: [],
            error: `Agent "${agentName}" is not a member of group "${msg.groupId}"`,
          });
          return;
        }

        const limit = Math.min(msg.limit || 50, 200);
        const messages = this.db.getGroupMessages(msg.groupId, limit);
        this.send(ws, {
          type: "group_history_response",
          requestId: msg.requestId,
          messages,
      ...
      if (msg.type === "group_members_request") {
        ...
        const enrichedMembers = members.map(m => {
          const agentRow = this.db.getAgentByName(m.agent_name);
          return {
            agent_name: m.agent_name,
            joined_at: m.joined_at,
            profile: agentRow ? parseProfile(agentRow.profile) : undefined,
          };
        });
      ...
      if (msg.type === "create_issue") {
        ...
        this.logger.info(`[mesh] Issue created by ${agentName}: "${msg.title}" (${issueId})`);

        // Issue stays unassigned — user must manually assign via dashboard.
        this.notifyIssueChanged(issueId, msg.groupId, "created");
      ...
      // ── Create collaboration (multi-agent collaboration issue) ──────────
      if (msg.type === "create_collaboration") {
        const conn = this.connections.get(agentId);
        const agentName = conn?.name || "unknown";

        // Owner 可选，若提供则必须是 "真人" 类型的注册 Agent
        if (msg.owner) {
          const ownerAgent = this.db.getAgentByName(msg.owner);
          if (!ownerAgent) {
            this.send(ws, {
              ...
              title: msg.title,
              status: "error",
              error: `Owner "${msg.owner}" is not a registered agent`,
            });
            return;
          }
          try {
            ...
            if (ownerProfile.category !== "真人") {
              this.send(ws, {
                ...
                title: msg.title,
                status: "error",
                error: `Owner must be a "真人" type agent`,
              });
              return;
            }
          } catch {
            this.send(ws, {
              ...
              title: msg.title,
              status: "error",
              error: `Owner profile parse error`,
            });
            return;
          }
        }

        // Validate participants are registered agents
        const allAgents = this.db.listAgents();
        const agentNames = new Set(allAgents.map((a) => a.name));
        const invalidParticipants = msg.participants.filter((p) => !agentNames.has(p));
        if (invalidParticipants.length > 0) {
          this.send(ws, {
            ...
            title: msg.title,
            status: "error",
            error: `Unknown agents: ${invalidParticipants.join(", ")}`,
          });
          return;
        }
        ...
        const issueId = randomUUID();
        this.db.createCollaborationIssue({
          id: issueId,
          groupId: msg.groupId,
          title: msg.title,
          collaborationGoal: msg.collaborationGoal,
        ...
        this.logger.info(`[mesh] Collaboration created by ${agentName}: "${msg.title}" (${issueId}), participants: ${msg.participants.join(", ")}, maxRounds: ${msg.maxRounds}, owner: ${msg.owner || "(none)"}`);

        // 协作只通知 participants[0]，由其自主决策 @ 下一个成员或结束 issue
        const firstParticipant = msg.participants[0];
        const firstAgent = this.db.getAgentByName(firstParticipant);
        if (firstAgent) {
          const grp = this.db.getGroupById(msg.groupId);
          this.sendToAgent(firstAgent.id, {
            ...
            round: 1,
            workingDir: grp?.working_dir || undefined,
          });
        }

        // 群内可见的启动消息：以 @<firstParticipant> 开头，
        // 让普通 agent 走标准 a2a_message + mention 路径被唤起；
        // 稳交付 worker 同时会通过上面的 collaboration_started 单独唤起。
        const ownerLine = msg.owner ? `\n负责人：${msg.owner}` : "";
        const startupContent =
          `@${firstParticipant} 🤝 [协作启动] 由你担任发起人，请开始协作任务「${msg.title}」\n` +
          `目标：${msg.collaborationGoal}\n` +
          `参与者：${msg.participants.join("、")}\n` +
          `最大轮数：${msg.maxRounds}` + ownerLine + `\n` +
          `IssueId：${issueId}`;
        this.postSystemToGroup(msg.groupId, startupContent, [], [firstParticipant]);
        this.notifyIssueChanged(issueId, msg.groupId, "created");
      ...
      // ── Conclude collaboration (Agent 主动结束协作) ─────────────────────
      if (msg.type === "conclude_collaboration") {
        const conn = this.connections.get(agentId);
        const agentName = conn?.name || "unknown";
        const issue = this.db.getIssueById(msg.issueId);
        if (!issue) {
          this.logger.warn(`[mesh] conclude_collaboration: issue ${msg.issueId} not found`);
          return;
        }
        if (issue.type !== "collaboration") {
          this.logger.warn(`[mesh] conclude_collaboration: issue ${msg.issueId} is not a collaboration`);
          return;
        }
        if (issue.status !== "in_progress") {
          this.logger.warn(`[mesh] conclude_collaboration: issue ${msg.issueId} not in progress (status=${issue.status})`);
          return;
        }
        const participants: string[] = JSON.parse(issue.participants || "[]");
        if (!participants.includes(agentName)) {
          this.logger.warn(`[mesh] conclude_collaboration: ${agentName} is not a participant of ${msg.issueId}`);
          return;
        }

        this.db.completeCollaboration(msg.issueId, msg.summary);
        ...
        for (const participant of participants) {
          const a = this.db.getAgentByName(participant);
          if (a) this.sendToAgent(a.id, conclusionMsg);
        }
        this.postSystemToGroup(issue.group_id, `🏁 [协作结束] 协作任务「${issue.title}」已由 ${agentName} 主动结束。\n${msg.summary}`);
        this.notifyIssueChanged(msg.issueId, issue.group_id, "updated");
        this.logger.info(`[mesh] Collaboration concluded by ${agentName}: "${issue.title}" (${msg.issueId})`);
        return;
      }
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // Issue system (稳交付组 task coordination)
  // ═══════════════════════════════════════════════════════════════════════════

  /** Push issue assignment notification to a specific 稳交付 Agent. */
