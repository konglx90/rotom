 ...
 *
 * NOTE: row types live in `./types.ts` so consumers can import them without
 * pulling in better-sqlite3. The runtime MeshDb class stays in this module
 * until method-by-method extraction lands in follow-up PRs.
 */
...
// Import row types for internal use AND re-export so callers that previously
// imported them from "../db.js" continue to work after the split.
import type {
  AgentRow,
  AuditLogRow,
  DomainRow,
...
export class MeshDb {
  ...
  private migrate(): void {
    ...
    // Resolve migration directory. The file now lives one directory deeper
    // (src/master/db/internal.ts), so add an extra `../` to compensate.
