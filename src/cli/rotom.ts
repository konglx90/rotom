...
// ── schedule ───────────────────────────────────────────────────────────────
// 群内定时任务：master 端 30s tick 扫描，到期任务派给 agent 或发到群里。
// CLI 走 /api/schedules；身份沿用现有解析（ROTOM_AGENT > --as > default）。
...
async function cmdSchedule(agent: ResolvedAgent, rest: string[], flags: Record<string, string | boolean>): Promise<void> {
  const sub = rest[0];
  ...
  if (sub === "add") {
    ...
    const body: Record<string, unknown> = {
      name, group_id: groupId, mode, prompt,
    };
  ...
  if (sub === "update") {
    const id = rest[1]; if (!id) fail("usage: rotom schedule update <id> [--every D] [--in D] [--at ISO] [--prompt T] [--name N] [--mode agent|message] [--agent A] [--repeat N] [--enabled true|false]");
    const body: Record<string, unknown> = {};
    ...
    const repeatRaw = flagStr(flags, "repeat");
    if (repeatRaw !== undefined) {
      if (repeatRaw === "0" || repeatRaw === "∞" || repeatRaw.toLowerCase() === "infinite") body.repeat_times = null;
      else {
        const n = parseInt(repeatRaw, 10);
    ...
    const atRaw = flagStr(flags, "at");
    const presentSchedule = [everyRaw, inRaw, atRaw].filter(Boolean).length;
    if (presentSchedule > 1) fail(`--every / --in / --at are mutually exclusive`);
    if (everyRaw !== undefined) {
      const sec = parseDuration(everyRaw);
