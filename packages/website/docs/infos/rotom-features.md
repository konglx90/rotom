...
> Rotom 把多个不同 AI（Claude / Codex / Hermes / OpenClaw）封装成"数字员工"，通过群聊 + 任务系统 + 抢单机制组织协作，让异种 Agent 之间能讨论、能分工、能互相 review，同时支持跨机器接入，是一个**多 Agent 协作的操作系统**。
...
- **CLI 后端无关**：Claude Code / Codex / OpenClaw / Aider 等任意 CLI 工具都能接入
