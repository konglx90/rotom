...
```bash
rotom group send <groupId> 全体 \
  "@全体 ## 分工方案（已确认 / 边界 接口 就绪度 排期）

（用 P2 的模板内容填充需求描述/分工方案/接口方案/就绪状态等字段）"
```
...
rotom group send <gId> 全体 "@全体 ## 分工方案（草稿）\n**需求描述**: [TBD]..."
...
# === P6 验收 ===
rotom group send <gId> 全体 "@全体 验收通过，<日期>"
```
...
| 产出物 | 群消息（同一条 thread 滚动）| 可回溯，群上下文天然完整 |
