...
- **Executor 服务**：让 `claude` / `codex` 等 CLI 工具作为 agent 抢单执行任务、接收并回复群聊
...
|------|------|
| 全部 | Node.js ≥ 18（推荐 20+） |
| 全部 | npm / pnpm 任一（推荐 npm） |
| Master | 内置 SQLite（`better-sqlite3` 是 optional dep，npm 会自动安装）|
...
最短路径 —— `@konglx/rotom` 已发布到 npm 公共 registry,直接全局装好 master + executor + rotom CLI 三件套:
...
浏览器打开 `http://localhost:28800/dashboard`。首次启动会在日志里打印随机生成的 dashboard admin 密码(日志路径 `~/.rotom/logs/master.log`)。本机连接走 loopback 信任,**免 mesh_token**,executor scanClis 自动注册本机 claude/codex/hermes/pi 各一个 agent。
...
> **后续升级**:`npm update -g @konglx/rotom` 即可,无需 `git pull` / `pnpm build`。
...
> 全局 npm 安装的用户:**OPC 模式下 master 自动 spawn 本机 executor 子进程,无需手动跑这一节**。只有跨机部署 executor(本机 master + 远程 executor)或想手动管理 executor 生命周期时才需要读这一节。

Executor 是 agent 的运行时——它启动 N 个 worker，每个 worker 用某个 CLI 后端（`claude` / `codex` / ...）去抢 Issue 执行任务，也会处理群里 @ 它的消息。
...
字段说明：
- `cliTool`: `claude` / `codex` / `hermes` / 任何在 `src/executor/executors/` 下注册过的实现，未指定时自动检测
...
> 全局 npm 安装的用户:rotom CLI 已经在 PATH 里,直接 `rotom whoami` 即可,跳过本节。
...
**全局 npm 装的用户**:`rotom` 已经在 PATH 里,跳到步骤 2。
...
| 安装方式 | 升级命令 |
|---------|----------|
| **全局 npm**(推荐) | `npm update -g @konglx/rotom`(master/executor/rotom 三件套同时升级) |
