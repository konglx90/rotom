        ...
        {result ? (
          <>
            <div className={s.tabBar}>
              <button className={`${s.tabBtn} ${tab === 'product' ? s.tabBtnActive : ''}`}
                onClick={() => setTab('product')}>{productLabel}</button>
              <button className={`${s.tabBtn} ${tab === 'process' ? s.tabBtnActive : ''}`}
                onClick={() => setTab('process')}>执行过程</button>
            </div>
            <div className={s.drawerBodyPadded}>
              {tab === 'product' ? (
                ...
                <IssueDetail issueId={issueId} agents={agents} groupMembers={[]} onBack={onClose} />
              )}
            </div>
          </>
        ) : (
