  ...
  return (
    <div className={s.sidebar}>
        ...
        {showCreate ? (
          <div className={s.sidebarCreateInner}>
            ...
            <div className={s.sidebarCreateActions}>
              <button onClick={handleCreate} className={s.ctaBtn}>创建</button>
              <button onClick={() => setShowCreate(false)} className={s.ghostBtn}>取消</button>
            </div>
          </div>
        ) : (
          <button onClick={() => setShowCreate(true)} className={s.ctaBtnFull}>+ 新建需求</button>
        )}
      </div>
      ...
      <div className={s.sidebarFooter}>
        <Link to="/dashboard/agents" className={s.sidebarFooterLink}>← 返回主面板</Link>
      </div>
    </div>
  )
}
  ...
  return (
    ...
    >
      ...
      <div className={s.reqItemInfo}>
        <div className={s.reqItemTitle}>{req.title || req.reqId.slice(0, 8)}</div>
