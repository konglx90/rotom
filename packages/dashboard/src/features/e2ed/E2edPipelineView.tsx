...
export function E2edPipelineView() {
  ...
  const createdAt = req.timeline?.[0]?.at
    ? new Date(req.timeline[0].at).toLocaleString()
    : "-";
            ...
            return (
              <div key={st} style={{ display: "flex", alignItems: "center" }}>
                <div className={s.flowNode}>
                  <div
                    className={`${s.flowDot} ${isActive ? s.flowDotActive : isPassed ? s.flowDotPassed : s.flowDotDefault}`}
                    style={
                      isActive || isPassed
                        ? {
                            background: c,
                ...
                {i < STATUS_FLOW.length - 1 && (
                  <div
                    className={`${s.flowLine} ${isPassed ? s.flowLinePassed : s.flowLineDefault}`}
                  />
                )}
              </div>
            );
        ...
        <div className={`${shared.card} ${shared.cardTopBlue}`}>
          ...
          <div className={s.timelineWrap}>
            ...
            {req.timeline.map((evt, i) => {
              ...
              const badgeCls2 =
                s[STATUS_BADGE[evt.status] as keyof typeof s] || s.pillGray;
              ...
              if (i > 0) {
                ...
                if (diff < 1000) duration = `${diff}ms`;
                  ...
                  duration = `${Math.floor(diff / 60000)}m ${Math.round((diff % 60000) / 1000)}s`;
                else
                  duration = `${Math.floor(diff / 3600000)}h ${Math.round((diff % 3600000) / 60000)}m`;
              }
              const sb = STATUS_BADGE[evt.status]
                ? {
                    bg: evt.status === "CLOSED" ? "#f3f4f6" : "#e2f6d5",
                    color: evt.status === "CLOSED" ? "#9ca3af" : "#054d28",
                  }
                : { bg: "#f1f5f9", color: "#64748b" };
              return (
                <div key={i} className={s.timelineRow}>
                  <div
                    className={`${s.timelineDot} ${isLast ? s.timelineDotActive : s.timelineDotDefault}`}
                    style={
                      isLast
                        ? {
                            background: sb.color,
                            border: `2px solid ${sb.color}`,
                            boxShadow: `0 0 0 3px ${sb.color}22`,
                          }
                        : undefined
                    }
                  />
                  ...
                  {duration && (
                    <span className={s.timelineDuration}>+{duration}</span>
                  )}
                </div>
              );
            })}
      ...
      {metrics && (
        <div className={`${shared.card} ${shared.cardTopAmber}`}>
          ...
          <div className={s.metricsGrid}>
            ...
            <MetricBox
              label="方案轮次"
              value={`${metrics.planRounds.length}`}
            />
            <MetricBox
              label="代码轮次"
              value={`${metrics.codeRounds.length}`}
            />
          </div>
          {metrics.planRounds.map((r) => (
            <div key={`mp${r.version}`} className={s.metricRow}>
              ...
              <span className={s.metricRowLabel}>
                交付 {(r.deliveryDuration / 1000).toFixed(1)}s
              </span>
              <span className={s.metricRowLabel}>
                评审 {(r.reviewDuration / 1000).toFixed(1)}s
              </span>
      ...
      {(req.source ||
        ...
        <div className={`${shared.card} ${shared.cardTopGray}`}>
          ...
          <div className={s.contextWrap}>
            ...
            {req.workingDir && (
              <ContextRow label="工作目录" value={req.workingDir} copyable />
            )}
            {req.links &&
              req.links.length > 0 &&
              req.links.map((l, i) => (
                <ContextRow
                  key={i}
                  label={l.type === "git-branch" ? "Git 分支" : l.type}
                  value={l.branch || l.url}
                  copyable
                />
              ))}
          </div>
  ...
  switch (status) {
    ...
    case "ENV_READY":
      return [
        {
          label: "生成方案",
          desc: "让 Claude 分析需求并生成实现方案",
          cmd: `rotom e2ed deliver ${groupId} --plan-only${cwd}`,
          accent: "green",
        },
        {
          label: "需求评审",
          desc: "让 Codex 独立评审需求质量",
          cmd: `rotom e2ed review ${groupId} --type requirement${cwd}`,
          accent: "purple",
        },
      ];
    case "ENV_BLOCKED":
      return [
        {
          label: "指定目录重试",
          desc: "环境检查失败，指定正确的项目目录重试",
          cmd: `rotom e2ed deliver ${groupId} --plan-only --cwd <正确目录>`,
          accent: "red",
        },
      ];
    case "REQ_REVIEWED":
    ...
    case "PLAN_REVIEWED":
      return [
        {
          label: "实现代码",
          desc: "方案已通过评审，让 Claude 编写代码",
          cmd: `rotom e2ed deliver ${groupId} --code-only${cwd}`,
          accent: "green",
        },
        {
          label: "再次方案评审",
          desc: "重新评审当前方案",
          cmd: `rotom e2ed review ${groupId} --type plan${cwd}`,
          accent: "purple",
        },
      ];
    case "DELIVERED":
      return [
        {
          label: "代码评审",
          desc: "代码已交付，让 Codex 评审代码质量",
          cmd: `rotom e2ed review ${groupId} --type code${cwd}`,
          accent: "green",
        },
        {
          label: "修复重交",
          desc: "基于上轮评审反馈修复并重新提交",
          cmd: `rotom e2ed deliver ${groupId} --code-only --fix${cwd}`,
          accent: "amber",
        },
        {
          label: "关闭需求",
          desc: "跳过评审，直接关闭",
          cmd: `rotom e2ed close ${groupId}`,
          accent: "purple",
        },
      ];
    case "REVIEWED":
          ...
          return (
            <div key={a.label} className={`${s.actionCard} ${cardCls}`}>
              <div className={s.actionHeader}>
                <span
                  className={s.actionTitle}
                  style={{ color: ACCENT_TITLE_COLOR[a.accent] }}
                >
                  {a.label}
                </span>
              </div>
