...
> Rotom 把多个不同 AI（Claude / Codex / DeepSeek / Hermes / OpenClaw）封装成"数字员工"，通过群聊 + 任务系统 + 抢单机制组织协作，让异种 Agent 之间能讨论、能分工、能互相 review，同时支持跨机器接入，是一个**多 Agent 协作的操作系统**。
...
Codex、Claude、DeepSeek 等是不同的"大脑"，内部思考问题的方式完全不一样。在特定任务下，Agent 之间的讨论可以碰撞出单一模型无法产出的方案。
...
- **CLI 后端无关**：Claude Code / Codex / OpenClaw / DeepSeek / Aider 等任意 CLI 工具都能接入
