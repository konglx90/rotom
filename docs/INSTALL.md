...
- `workingDir`: **必填**,本机可读的项目目录。Agent 在该目录下**只读**访问(Read / Grep / Glob / Bash 只读命令),不得 Write / Edit。**跨机器部署时,每台 executor 各自配置自己机器上的项目路径,不需要与 master 共享 FS**。master 通过 WS 推过来的 `working_dir` 会被 executor 忽略,只以本机这份配置为准。启动时校验路径存在 / 可读,缺失或不合法会 fail-fast。
